...
class ShowPresenter < Blacklight::ShowPresenter
  ...
  ######### TEST dynamic 
  # CONFIG_TYPES = ['primary', 'phys_desc', 'topic', 'geo_loc', 'coll_info', 'identifiers', 'use']
  
  # CONFIG_TYPES.each do |types|
  #   define_method(types) do 
  #     fields_to_render.each do |field_name, field_config, field_presenter|
  #       field_presenter.except_operations << Blacklight::Rendering::Join
  #       # return field_name, field_config, field_presenter if field_config[:type] == :"#{types}"
  #       shart = Proc.new {|field_name, field_config, field_presenter| field_config[:type] == :"#{types}"}
  #       shart
  #     end
  #   end
  # end
